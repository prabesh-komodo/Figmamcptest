...
export default class WorkflowNode extends LightningElement {
  ...
  get isTransition() {
    return this.nodeType === "transition";
  }
  get isDeletable() {
    return this.nodeType !== "root";
  }
  ...
  get chevronClass() {
    return (
      "chevron-icon" + (this.isCollapsed ? " chevron-icon--collapsed" : "")
    );
  }
  ...
  get inputPortClass() {
    return (
      "node-input-port" +
      (this.isConnectTarget ? " node-input-port--target" : "")
    );
  }
  ...
  get typeLabel() {
    return this.nodeData
      ? NODE_TYPE_LABELS[this.nodeData.nodeType] || "Step"
      : "";
  }
  ...
  get fields() {
    ...
    switch (d.nodeType) {
      case "root":
        ...
        result.push({
          label: "Criteria Fact",
          value: d.criteriaFact || "\u2014"
        });
        break;
      case "stage":
        result.push({
          label: "Lifecycle State",
          value: d.lifecycleState || "\u2014"
        });
        result.push({
          label: "Custom Label API",
          value: d.customLabelApiName || "\u2014"
        });
        break;
      case "transition":
        break;
  ...
  get autostartBadgeClass() {
    return (
      "badge" +
      (this.nodeData && this.nodeData.autostart ? " badge--on" : " badge--off")
    );
  }
  ...
  get isActiveBadgeClass() {
    return (
      "badge" +
      (this.nodeData && this.nodeData.isActive ? " badge--on" : " badge--off")
    );
  }
  ...
  get manualTransitionLabel() {
    return this.nodeData && this.nodeData.allowManualTransition
      ? "Allowed"
      : "Not allowed";
  }
  ...
  get manualTransitionBadgeClass() {
    return (
      "badge" +
      (this.nodeData && this.nodeData.allowManualTransition
        ? " badge--on"
        : " badge--off")
    );
  }
  ...
  get finalStageBadgeClass() {
    return (
      "badge" +
      (this.nodeData && this.nodeData.isFinalStage
        ? " badge--on"
        : " badge--off")
    );
  }
