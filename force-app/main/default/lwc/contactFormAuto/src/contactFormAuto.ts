...
export default class ContactFormAuto extends LightningElement {
    ownerName = '';
    ownerId = '';
    ...
    wiredObjectInfo({
        ...
        data?: { defaultRecordTypeId?: string };
        error?: Error;
    }) {
        if (data) {
    ...
    handleReportsToChange(event: CustomEvent): void {
        const detail = event.detail as { recordId?: string };
        this.reportsToId = detail?.recordId ?? '';
    }

    handleLeadSourceChange(event: CustomEvent): void {
        const detail = event.detail as { recordId?: string };
        this.leadSourceId = detail?.recordId ?? '';
    }
    ...
    private validateForm(): boolean {
        ...
        inputs.forEach((el: Element) => {
            ...
            if (typeof inputEl.reportValidity === 'function') {
                valid = inputEl.reportValidity() && valid;
            }
        });
        return valid;
    }
